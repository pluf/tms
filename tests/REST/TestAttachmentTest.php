...
class TestAttachmentTest extends AbstractTest
{
    ...
    {
        $client = new Client();
        $response = $client->get('/tms/test-attachments/schema');
    ...
    {
        $client = new Client();
        $response = $client->get('/tms/test-attachments');
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    }
    ...
    public function gettingListOfProjectsByGraphQLTest()
    {
        ...
        // 2- getting list of projects
        $response = $client->get('/tms/test-attachments', array(
    ...
    public function createATestWithAdminTest()
    {
        ...
        $response = $client->post('/tms/test-attachments', $data);
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    }
    ...
    public function deleteATestWithAdminTest()
    {
        ...
        $response = $client->post('/tms/test-attachments/' . $id, array(
            'graphql' => '{id,test{id},test_id}'
        ));
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);

        $response = $client->delete('/tms/test-attachments/' . $id);
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
