...
class REST_TestAcceptanceCriterionTest extends AbstractTest
{
    ...
    {
        $client = new Client();
        $response = $client->get('/tms/test-acceptance-criteria/schema');
    ...
    {
        $client = new Client();
        $response = $client->get('/tms/test-acceptance-criteria');
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    }
    ...
    public function gettingListOfActivitiesByGraphQLTest()
    {
        ...
        // 2- getting list of projects
        $response = $client->get('/tms/test-acceptance-criteria', array(
    ...
    public function createATestActivityWithAdminTest()
    {
        ...
        $response = $client->post('/tms/test-acceptance-criteria', $data);
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    }
    ...
    public function deleteATestWithAdminTest()
    {
        ...
        $response = $client->post('/tms/test-acceptance-criteria/' . $id, array(
            'graphql' => '{id,test{id},test_id}'
        ));
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);

        $response = $client->delete('/tms/test-acceptance-criteria/' . $id);
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
