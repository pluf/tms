...
class REST_TestTest extends AbstractTest
{
    ...
    {
        $client = new Client();
        $response = $client->get('/tms/tests/schema');
    ...
    public function gettingListOfProjectsByGraphQLTest()
    {
        ...
        // 2- getting list of projects
        $response = $client->get('/tms/tests', array(
            'graphql' => '{items{id,project{id}}}'
        ));
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    }
    ...
    public function createATestWithAdminTest()
    {
        ...
        $response = $client->post('/tms/tests', $data);
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    }
    ...
    public function deleteATestWithAdminTest()
    {
        ...
        $response = $client->post('/tms/tests/' . $id, array(
            'graphql' => '{id,project{id}}'
        ));
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);

        $response = $client->delete('/tms/tests/' . $id);
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    ...
    public function getATestHistoryWithAdminTest()
    {
        ...
        $response = $client->get('/tms/tests/' . $id . '/histories');
        $this->assertNotNull($response);
        $this->assertEquals($response->status_code, 200);
    }
}
